    ...
    <div class="flex flex-col w-64 h-screen px-4 py-8 bg-white border-r dark:bg-gray-800 dark:border-gray-600">
      ...
      <div class="flex flex-col justify-between flex-1 mt-6">
        <nav>
          ...
          <a :class="{ 'bg-active': active }" @click="accountsClick()"
          ...
          <a @click="statisticClick()"
            ...
            <span class="mx-4 font-medium">Statistic</span>
          </a>
        ...
        <div class="flex items-center px-4 -mx-2">
          <img class="object-cover mx-2 rounded-full h-9 w-9"
               ...
               alt="avatar"/>
          <h4 class="mx-2 font-medium text-gray-800 dark:text-gray-200 hover:underline">{{ user.first_name }}
            {{ user.last_name }}</h4>
        </div>
      </div>
    </div>
    <div class="flex-1 contentBox">
      <app-header/>
      <div class="p-8">
        <HomeDashboard :user="this.user" v-if="home"/>
        <FileList :user="this.user" v-if="document"/>
        <UserProfil v-if="accounts"/>
        <NotiFication v-if="notification"/>
        <StateImp v-if="statistic"/>
        <AppSettings v-if="settings"/>
      </div>
    </div>
  </div>
